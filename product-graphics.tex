...
Impossible to simultaneously satisfy all constraints.  Every product plot requires the creator to trade off between these.  We will focus our attention on rectangular partitions.  Special cases of $h(i, j) = c(i, j)^x$ $w(i, j) = c(i, j)^y$,   $x + y = 1$.
