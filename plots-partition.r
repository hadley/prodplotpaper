...
# year & health
prodplot(happy, ~ . | year + health, "fluct", na.rm = TRUE) + 
  aes(fill = health)
...
draw(subset(df, level == 1)) + aes(fill = marital)
ggsave("plots/part-comb-1.pdf", width = 6, height = 6)

draw(subset(df, level == 2)) + aes(fill = sex)
...
prodplot(happy, ~ decade + marital, set_offset(c("hbar", "vspine"), c(0, 0.02)), na.rm = T) + aes(fill = marital)
